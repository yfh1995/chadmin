...
use backend\services\AdminActionService;
use Yii;
...
class BaseController extends Controller
{
    ...
    public function beforeAction($action)
    {
        if (parent::beforeAction($action)) {
            $this->permissions = Yii::$app->user->isGuest ? [] : Yii::$app->user->identity->getSystemRights();
    ...
    private function verifyPermission($action){
        ...
        if(Yii::$app->user->isGuest){
            ...
            if(in_array($route, $loginAllowUrl) == false){
               if((empty($this->permissions) == true || empty($this->permissions[$route]) == true)){
//                    header("Content-type: text/html; charset=utf-8");
//                    exit('没有权限访问'.$route);
                   return $this->redirect(Yii::$app->request->referrer)->send();
               }
               $rights = $this->permissions[$route];
    ...
    protected function getAllControllerAction(){
        $caDatas = AdminActionService::getFields('controller, action', [], ['display_order' => 'asc']);
        $rightActionData = [];
        foreach($caDatas as $v){
            if (!isset($rightActionData[$v['controller']])) {
                $rightActionData[$v['controller']] = [
                    'text'      =>  $v['controller'],
                    'selectable'=>  false,
                    'state'     =>  [
                        'checked'   =>  false
            ...
            $controllerName = substr($v['controller'], 0, strlen($v['controller']) - 10);
            $aUrl = $v['action'];
            $cUrl = Inflector::camel2id(StringHelper::basename($controllerName));
            $rightActionData[$v['controller']]['nodes'][] = [
                'text'  =>  "$aUrl&nbsp;&nbsp;($cUrl/$aUrl)",
                'c'     =>  $cUrl,
                'a'     =>  $aUrl,
                'selectable'    =>  true,
                'state' =>  [
                    'checked'   =>  isset($rightUrls[$cUrl.'/'.$aUrl]) ? true : false
                ],
                'type'  =>  'a'
            ];
            if(isset($rightUrls[$cUrl.'/'.$aUrl]) == true){
                $rightActionData[$v['controller']]['state']['checked'] = true;
            }
        }
        return $rightActionData;
    }

//    protected function have
